...
class SwinIR(nn.Module):
    ...
    def __init__(self,
        ...
        # self.L1 = MyModel(3,64,256)
        # self.L2 = MyModel(3, 64, 256)
        # self.L3 = MyModel(3, 64, 256)
        ...
        # self.conv_blast = nn.Conv2d(num_feat, num_out_ch * 256, 3, 1, 1)
    ...
    def softargmax(x, beta=1e10):
        ...
        return torch.sum(indices * x_softmax, dim=1)

    def binary_to_decimal(self, binary_tensors):
        """将8个形状为 (b, c, h, w) 的二进制张量列表重新组合成一个形状为 (b, c, h, w) 的十进制张量"""
        b, c, h, w = binary_tensors[0].shape

        decimal_tensor = torch.zeros(b, c, h, w, dtype=torch.uint8, device=binary_tensors[0].device)
        # for i in range(8):
        #     print("b shape------", binary_tensors[i].shape)
        for i in range(8):

            decimal_tensor += (binary_tensors[i].to(torch.uint8) << i)
    ...
    def reshape_and_split(self, x_b):
        ...
        # 重新调整形状为 b, 8, 3, h, w
        x_b_reshaped = x_b.view(b, 8, 3, h, w)

        # 拆分成 8 个形状为 b * 1 * 3 * h * w 的张量
        split_tensors = torch.split(x_b_reshaped, 1, dim=1)

        # 移除多余的维度，使每个张量的形状为 b * 3 * h * w
